...
def get_all_ingredients():
    try:
        cursor.execute('SELECT ingredient, expiry_date FROM ingredients ORDER BY expiry_date ASC')
        return cursor.fetchall()
    except Exception as e:
        st.error(f"❌ Database Error: {e}")
        return []
...
# Button to show all ingredients
if st.button("📋 Show Ingredients"):
    all_ingredients = get_all_ingredients()
    if all_ingredients:
        st.subheader("📌 Stored Ingredients")
        for ingredient, expiry_date in all_ingredients:
